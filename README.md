...
| Layer | Tool | Why |
|-------|------|-----|
| Language | TypeScript | Type safety and better tooling support |
...
# Or watch TypeScript files for changes during development
npm run dev:ts
```
...
│   ├── proxy.ts           # Fetch → translate → rewrite HTML pipeline
│   ├── translator.ts      # Google Translate wrapper (batch + single)
│   └── image-translator.ts # OCR → translate → canvas overlay pipeline
